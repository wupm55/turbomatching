...
flattenDataset[dataset_Dataset]:=Module[{info,pos,normalParts,subPartsKey,subParts},
info=dataset//Normal;
...
compareTest[ids_List,x_,y_,showData_:False]:=Module[{results,info,pane},
...
easyCompareTests:=Manipulate[compareTest[tests,x,y,showData],{x,resultKeys},{{y,"torque"},DeleteCases[resultKeys,"speed"]},{{showData,False,"Show Numbers"},{True,False}},{tests,ListPicker[#1,testList]&,ControlPlacement->Left}]
...
getCMapPlotData[id_,style:OptionsPattern[]]:=Module[{map,pressLine,etaLine,surgeLine,nred,uredC,pressLineSpeed,etaLineSpeed,tip,d2,surge,pr,eta},
map=getMapData[id];
...
plotLugLinesOnMaps[testids_List,mapids_List,showContour_,opt:OptionsPattern[]]:=Module[{mapPlots,mapTPlots,testC,testT,c,t},
...
plotCMaps[ids_List,showContour_,opt:OptionsPattern[]]:=Module[{maps,n,styles,mapdatas},
n=Length[ids];
...
{{Show[Show[Table[plotCMapContour[mapdatas[[i]]],{i,1,n}]],
  plotSets[Flatten[Table[{maps[[i]]["pr"],maps[[i]]["surge"]},{i,1,n}],1],
  AxesLabel->{"Vred[\!\(\*SuperscriptBox[\(m\), \(3\)]\)/s]","p2p1"},
    PlotLabel->"Compressor Map \!\(\*SubscriptBox[\(V\), \(red\)]\)-PI"],PlotRange->All,opt],
  plotSets[Table[maps[[i]]["eta"],{i,1,n}],AxesLabel->{"Vred","\!\(\*SubscriptBox[\(\[Eta]\), \(c\)]\)"},PlotLabel->"Compressor Map \!\(\*SubscriptBox[\(V\), \(red\)]\)-\!\(\*SubscriptBox[\(\[Eta]\), \(c\)]\)",opt][[1]]
},
datasetConnect[
	getMapInfo/@ids
]}
	...
	PlotLabel->"Compressor Map \!\(\*SubscriptBox[\(V\), \(red\)]\)-PI",PlotRange->All,opt],
	plotSets[Table[maps[[i]]["eta"],{i,1,n}],AxesLabel->{"Vred[\!\(\*SuperscriptBox[\(m\), \(3\)]\)/s]","\!\(\*SubscriptBox[\(\[Eta]\), \(c\)]\)"},PlotLabel->"Compressor Map \!\(\*SubscriptBox[\(V\), \(red\)]\)-\!\(\*SubscriptBox[\(\[Eta]\), \(c\)]\)",opt][[1]]
...
easyMaps:=Manipulate[plotMaps[id,showContour],{id,ListPicker[#,mapList]&},{{showContour,False,"show contour"},{True,False}},ControlPlacement->Left]
